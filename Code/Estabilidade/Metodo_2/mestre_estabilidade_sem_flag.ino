...
#							                                          #
#	                  Mestre - ESP-NOW                    #
#            Verifica Estabilidade - Método 2		        #
#							                                          #
#	     	                                                #
...
Utilizando o método 2, o mestre só envia o próximo pacote se
o número do pacote atual for igual ao número do pacote anterior
somado com 1.
...
int pacotes = 20000;                  //indica quantidade de pacotes a serem enviados. Note que foi colocado um número maior de pacotes, 
...
esp_now_peer_info_t peerInfo;  //cria uma estrutura que será utilizada para registrar informações 
...
void loop() {   
    ...
    int c_old = -1;                               //declara variavel que irá armazenar o número do último pacote enviado
    while(c<= pacotes){                           //envia 1000 pacotes de 250 bytes
        if(c >= (c_old + 1)){                     //se o pacote a ser enviado for igual ao pacote anterior mais 1
            dataSend.a[0] = c;                    //a estrutura de envio na posição 0 recebe o valor do número do pacote
            c_old = c;                            //a variavel c_old recebe o novo valor do pacote
