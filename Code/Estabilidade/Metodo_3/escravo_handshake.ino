...
Utilizando o método 3, o escravo pode verificar a quantidade
de perdas, se existe inversão de pacote durante a transmissão
e se o conteúdo do pacote é alterado.
Além disso, ele envia para o mestre a confirmação de que um pacote
foi recebido. 
...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len) {
  ...
  if((printa_uma_vez == 1)){
    ...
    dataSend.a[0] = 1;                                                              //confirma que pacote foi recebido
    esp_now_send(macAddress1Esp32, (uint8_t *) &dataSend, sizeof(dataSend));        //envia para o mestre a confirmação
    return;
  }
