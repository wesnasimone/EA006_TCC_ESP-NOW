...
#							                                          #
#	                  Escravo - ESP-NOW		                #
#							 Verifica Estabilidade - Método 4         #
#	     	                                                #
...
Utilizando o método 4, o escravo pode verificar a quantidade
de perdas, se existe inversão de pacote durante a transmissão
e se o conteúdo do pacote é alterado.
Além disso, o escravo envia para o mestre uma confirmação de 
que recebeu o pacote e se o pacote recebido está correto ou não.
...
int flag_recontagem = 0;                //define variavel para sinalizar se já iniciou uma nova leva de transmissão
...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len) {
  ...
  if((printa_uma_vez == 1)){
      ...
      //então o novo dado não é permitido ser armazenado no vetor_recebido, mas ainda sim 
      //é necessário informar ao mestre que o dado chegou e que chegou corretamente
      if(flag_recontagem == 0){
          dataSend.a[0] = 1;
          dataSend.a[1] = 2;
          esp_now_send(macAddress1Esp32, (uint8_t *) &dataSend, sizeof(dataSend));
          return;
      }
  ...
  //verifica se o número recebido corresponde ao mesmo número do contador count
  if(vetor_recebido[count] == count){
      count = count + 1;                //se sim, incrementa a variavel para que o proximo valor recebido
                                        //possa ser armazenado na proxima posição do vetor
      dataSend.a[1] = 2;                //confirma que o dado foi recebido corretamente
  }
  else{
    dataSend.a[1] = count;              //caso contrário envia o valor do contador para o mestre
  }
   

  dataSend.a[0] = 1;                                                            //sinaliza que o dado foi recebido
  esp_now_send(macAddress1Esp32, (uint8_t *) &dataSend, sizeof(dataSend));      //envia confirmação para o mestre
 }
